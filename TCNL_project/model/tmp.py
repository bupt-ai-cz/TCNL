...
class SE(nn.Module):
    ...
    def forward(self, x):
        ...
        out = F.sigmoid(out)
        weight = torch.ones(size=out.shape).cuda()
        for i in range(out.shape[0]):
            weight[i] = torch.where(out[i]>torch.mean(out[i]), 1, 0)
        out = x * weight
...
class vgg_multi_concept_for_mammal(nn.Module):
    def __init__(self, args: dict):
        ...
        # self.head_feature_mask = SE(in_chnls=512)
        # self.head_feature_mask = layer_mask()
        ...
        # self.torso_feature_mask = SE(in_chnls=512)
        # self.torso_feature_mask = layer_mask()
        self.leg_feature_extractor = vgg_deep(num_classes=args['num_classes'])
        # self.leg_feature_mask = SE(in_chnls=512)
        # self.leg_feature_mask = layer_mask()
        ...
        # self.optimizer_head_mask = torch.optim.SGD(self.head_feature_mask.parameters(), lr=0.001)
        # self.optimizer_torso_mask = torch.optim.SGD(self.torso_feature_mask.parameters(), lr=0.001)
        # self.optimizer_leg_mask = torch.optim.SGD(self.leg_feature_mask.parameters(), lr=0.001)
    ...
    def forward(self, x, label, head, torso, leg, shape):
        ...
        # head_feature = self.head_feature_mask(head_feature)
        torso_feature = self.torso_feature_extractor(shallow_feature)
        # torso_feature = self.torso_feature_mask(torso_feature)
        leg_feature = self.leg_feature_extractor(shallow_feature)
        # leg_feature = self.torso_feature_mask(torso_feature)
    ...
    def backward_for_mask(self, reg):
        reg.backward()
    ...
    def optimize_parameters(self, loss_C, loss_R_head, loss_D_head, loss_R_shape, loss_D_shape, loss_R_torso, loss_D_torso, loss_R_leg, loss_D_leg, flag):
        ...
        if flag == 'all':
            ...
            # set_requires_grad(self.head_feature_mask, False)
            # set_requires_grad(self.torso_feature_mask, False)
            # set_requires_grad(self.leg_feature_mask, False)
            ...
            # set_requires_grad(self.head_feature_mask, True)
            # set_requires_grad(self.torso_feature_mask, True)
            # set_requires_grad(self.leg_feature_mask, True)
...
class VGG(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.fc3(out), feature.cpu()
...
class vgg_multi_concept_for_scene(nn.Module):
    def __init__(self, args: dict):
        super(vgg_multi_concept_for_scene, self).__init__()
        # block
        self.shallow_backbone = vgg_shallow(args['num_classes'])
        self.bed_feature_extractor = vgg_deep(num_classes=args['num_classes'])
        ...
        self.sofa_feature_extractor = vgg_deep(num_classes=args['num_classes'])
        ...
        self.shelf_feature_extractor = vgg_deep(num_classes=args['num_classes'])
        self.seat_feature_extractor = vgg_deep(num_classes=args['num_classes'])
        ...
        self.rest_feature_extractor = vgg_deep(num_classes=args['num_classes'])

        self.classifier = VGG_Classifier(num_classes=args['num_classes'])
        self.reconstructor_bed = VGG_Reconstructor()
        self.discriminator_bed = VGG_Discriminator()
        ...
        self.reconstructor_sofa = VGG_Reconstructor()
        self.discriminator_sofa = VGG_Discriminator()
        ...
        self.reconstructor_shelf = VGG_Reconstructor()
        self.discriminator_shelf = VGG_Discriminator()
        self.reconstructor_seat = VGG_Reconstructor()
        self.discriminator_seat = VGG_Discriminator()
