...
import shutil
import wandb
from model.new_inception import new_inception
...
def train(epoch: int, net: new_inception, train_loader, args: dict, train_loss: list, train_acc: list):
    ...
    for index, data in enumerate(train_loader):
        ...
        target_shape_dir = '/root/XCSGCNN/train/visulization/new_inception/train/outline/' + str(epoch)
        target_head_dir = '/root/XCSGCNN/train/visulization/new_inception/train/head/' + str(epoch)
...
def val(epoch: int, net: new_inception, val_loader, args: dict, val_loss: list, val_acc :list):
    global best_acc
    global best_loss
    net.eval()
    loss, correct, total = 0, 0, 0
    ...
    with torch.no_grad():
        for index, data in enumerate(val_loader):
            ...
            image_name = data['image_name']
            pred_class, shape_pred_fake, shape_pred_real, head_pred_fake, head_pred_real, reconstruct_shape, reconstruct_head, object, head = net(image, label, object, head)
            target_shape_dir = '/root/XCSGCNN/train/visulization/new_inception/val/outline/' + str(epoch)
            target_head_dir = '/root/XCSGCNN/train/visulization/new_inception/val/head/' + str(epoch)
            array_to_image(image_name, reconstruct_shape.cpu().data.numpy(), target_shape_dir)
            array_to_image(image_name, reconstruct_head.cpu().data.numpy(), target_head_dir)
...
if __name__ == '__main__':
    ...
    args = {
        ...
        'ckpt_path': '/root/XCSGCNN/train/ckpt/new_inception',
        'train_batch_size': 8,
        'val_batch_size': 8,
        'device': 'cuda:0',
        'epoch': 500,
    ...
    block_file = '/root/XCSGCNN/train/model/inception_backbone.py'
    model_file = '/root/XCSGCNN/train/model/new_inception.py'
